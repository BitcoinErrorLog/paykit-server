...
| `max_utxos_per_address` | `200` | > 0, and items × 160 B + envelope ≤ `max_response_bytes` | Decoded list_unspent item cap per address; startup refuses an item cap whose maximum reply would exceed the transport byte cap. |

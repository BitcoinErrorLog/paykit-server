...
| `max_utxos_per_address` | `200` | > 0, and items × 176 B + envelope ≤ `max_response_bytes` | Decoded list_unspent item cap per address; startup refuses an item cap whose maximum reply would exceed the transport byte cap. |
| `address_deadline` | `5s` | > 0 | Per-address wall-clock deadline, shared by the tick's probe. |

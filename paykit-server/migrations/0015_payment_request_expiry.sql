...
-- As introduced here the `expired_tail → expired_final` edge did NOT
-- terminalize the invoice's non-handed-off outbox rows (void/abandonment
-- did); that gap is closed in the persistence layer — the expiry transition
-- terminalizes them in the same transaction with reason `invoice_expired`,
-- and a bounded one-time backfill sweep (reason `invoice_final_backfill`)
-- drains rows left inert by invoices that expired before the close.

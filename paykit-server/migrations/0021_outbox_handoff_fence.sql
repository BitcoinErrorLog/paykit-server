...
-- completion is recovered by the ordinary claim path, which re-admits a
-- `handoff_started` row once its lease expires. The terminal-row repair
-- trigger is unaffected: it guards `permanently_failed` rows only.

...
--
-- claim_channel is the verbatim `claim_channel` the Shop client submitted on
-- POST /v0/accounts/claim ('manual' or 'bitkit_watch_only_v1'; B.8.6); NULL
-- for creators whose claim predates the field or came through the Bitkit
-- companion flow, which carries no channel assertion and performs no
-- claim-time scan. downgrade_reason is one of B.8.8's five fixed
-- identifiers; NULL when no downgrade reason was ever assigned.
